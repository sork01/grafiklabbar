...
// *neode.command* setgo gcc -O3 -I/usr/include/SDL -I./glm -lstdc++ -lm -lSDL lab2final.cpp -o lab2final; ./lab2final
...
float focal_length = SCREEN_HEIGHT / 1.25f;
vec3 camera_pos(0.0f,0.0f,-2.51f);
...
vec3 light_pos(0.0f, -0.1f, -0.7f);
vec3 light_color = 14.0f * vec3(1.0f, 1.0f, 1.0f);
vec3 indirect_light = 0.33f * vec3(1, 1, 1);
...
vec3 directLight(const intersection& i)
{
    ...
    {
        float thedot = max(0.0f, glm::dot(triangles[i.triangle_index].normal, glm::normalize(vr)));
        
        return vec3(light_color * thedot) / (4.0f * 3.1415f * r*r);
    }
}

int main(int argc, char* argv[])
{
    printf("hfov is %.2f degrees\n", (2*atan(float(SCREEN_WIDTH) / (2.0*focal_length))) * (180.0 / 3.1415));
    printf("vfov is %.2f degrees\n", (2*atan(float(SCREEN_HEIGHT) / (2.0*focal_length))) * (180.0 / 3.1415));
