...
// *neode.command* setgo gcc -O3 -I./glm -I/usr/include/SDL -lstdc++ -lm -lSDL -lSDL_image normaltest.cpp -o normaltest; ./normaltest
#include <iostream>
#include <vector>
#include <string>
#include <glm/glm.hpp>
...
public:
    ...
    Texture(string filename, string normalmap)
    {
        surface = IMG_Load(filename.c_str());
        normalsurf = IMG_Load(normalmap.c_str());
        ...
        normalwidth = ((float)surface->w) - 1.0f;
        normalheight = ((float)surface->h) - 1.0f;
        
        pixels = (uint8_t*) surface->pixels;
        normalpixels = (uint8_t*) normalsurf->pixels;
    }
    ...
    vec3 GetNormalAt(vec2 coords)
    {
        ...
        offset = y * normalsurf->pitch + 4*x;
        
        vec3 result(-1.0f + 2.0f*((float)normalpixels[offset] / 255.0f), -1.0f + 2.0f*((float)normalpixels[offset+1] / 255.0f), -(float)normalpixels[offset+2] / 255.0f);
...
int main(int argc, char* argv[])
{
    ...
    Texture* logo = new Texture("kth.png", "kth-normal.png");
...
void PixelShader(const Pixel& p, vec3 color)
{
    ...
    float thedot = glm::dot(p.texture->GetNormalAt(p.textureCoords / p.zinv) * cameraRotation, glm::normalize(r));
    ...
    // vec3 illumination = p.texture->GetNormalAt(p.textureCoords / p.zinv) * ((lightPower * max(thedot, 0.0f)) / (4.0f * 3.14159265f * rmagn)) + indirectLightPowerPerArea;
