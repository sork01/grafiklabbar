...
// *neode.command* setgo g++ objloader.h -o /tmp/objloader
...
void LoadOBJModel(string filename, vector<Triangle>& triangles, float scale = 1.0f, bool flipY = false)
{
    ifstream data;
    data.open(filename);
    ...
    while (data.good())
    {
        ...
        else if (op == "v")
        {
            ...
            if (flipY)
            {
                vertices.push_back(glm::vec3(v0, -v1, v2) * scale);
            }
            else
            {
                vertices.push_back(glm::vec3(v0, v1, v2) * scale);
